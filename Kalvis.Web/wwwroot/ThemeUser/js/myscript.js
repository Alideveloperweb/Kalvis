...
$(document).on('click', function (event) {
  ...
  if (!$(event.target).closest('.navigation').length && !$(event.target).closest('.menu-icon').length) {
...
$('.main-menu.has-sub > a').on('click', function (){
  var _this=$(this).parent();

  if(_this.hasClass('open')){
    _this.removeClass('open');
    _this.find('.sub-menu').removeClass('is-active');
  }
  else{
    _this.addClass('open');
    _this.find('.sub-menu').addClass('is-active');
  }
})
